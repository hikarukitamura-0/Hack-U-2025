 ...
 */
export const onNewSwipeCheckMatch = functionsV1
  .firestore.document('swipes/{swipeId}')
  .onCreate(async (snapshot: DocumentSnapshot, context: EventContext) => { 
    
    const newSwipe = snapshot.data() as FirebaseFirestore.DocumentData;
    const swiperUid = newSwipe.swiper_uid as string;
    const swipedUid = newSwipe.swiped_uid as string;
    const action = newSwipe.action as string;
 ...
 */
export const getHobbySyncRate = functionsV1.https.onCall(async (data, context) => {
  // 1. 認証チェック
  if (!context.auth) {
  ...
  try {
    ...
    // データがない場合のデフォルト値
    if (!userData) return { syncRate: 50, reason: "プロフィール設定がまだのようです。" };
    if (!hobbyData) return { syncRate: 50, reason: "趣味データが見つかりません。" };
